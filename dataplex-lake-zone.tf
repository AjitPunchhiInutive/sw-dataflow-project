        ...
        zone_key => merge(zone_val, {
          assets = {
            ...
            asset_key => merge(asset_val, {
              ...
              # ── Resolve cron here — null/"inherited" → inherit from zone ──
              cron_schedule = (
                try(asset_val.cron_schedule, null) == null ||
                try(asset_val.cron_schedule, null) == "inherited"
                ? try(zone_val.cron_schedule, null)  # ← resolved from zone
                : asset_val.cron_schedule            # ← use asset's own value
              )
            })
          }
        })
