locals {
  _dataplex_raw = { for f in fileset("${path.module}/config/dataplex", "*.yaml") : trimsuffix(f, ".yaml") => yamldecode(file("${path.module}/config/dataplex/${f}"))
  }

  # ── Guard: surface any file that is missing required keys ───────────────
  # If this validation fires, the named file has the wrong structure.
  _dataplex_validation = { for key, val in local._dataplex_raw :
    key => {
  ...
  dataplex_configs = { for key, val in local._dataplex_raw :  key => val   if can(val.project_id) && can(val.region) && can(val.zones) }
}
...
module "dataplex" {
  ...
  name       = each.key
  prefix     = try(each.value.prefix)       # safe default if missing
